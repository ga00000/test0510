...
$(document).ready(function(){
	...
	$(".ico.twitter").bind("click", function(){
...
function fncCmmnRequestTot( searchCondition ){
...
var setUploadData = function(result){
	if(result.result){
		...
		$.each(result.resultList, function (intIndex, strValue) {
			...
			innerHtml += "<li>";
			innerHtml += "<a href=\"javascript:fncFileDownload('"+thisData.atchFileId+"', '"+thisData.fileSn+"', '"+thisData.streFileNm+"');\" class=\"ico file\"><span class=\"ir ico_file\">첨부파일 있음</span>"+thisData.orignFileNm+"("+thisData.fileSize+")</a>";
			innerHtml += "</li>";
...
var fncEmailQuestion = function(manageId){
...
var fncQuestion = function(divnCd){
...
var fncApplyQuestion = function(branchCd, divnCd){
...
//에너지팜 렌탈, 영어강의 신청하기 팝업 parameter = rental, lecture
...
//공감레터, ENERZINE 사보 신청 레이어팝업 parameter divnCd=newsletter,enerzine, typeCd=insert,update
...
//문자알림 레이어 팝업, parameter = insert,update
   	...
   	$.getJSON("/function/manage/list.json?menuCd=" + $("#pageMenuCd").val(), function (result) {
   		if(typeof(result.manageList) == "object"){
    		...
    		$.each(result.manageList, function (intIndex, strValue) {
				...
				var strHtml = "<tr>"
				+ "	<td><div class=\"tdCont\">"+thisData.branchNm+" "+thisData.deptNm+" / "+thisData.empNm+"</div></td>"
                + "	<td><div class=\"tdCont\">"+thisData.telNo+"</div></td>"
                + "	<td><div class=\"tdCont\">평일 09:00 ~ 18:00</div></td>"
                + "	<td><div class=\"tdCont\"><a href=\"#\" onclick=\"javascript:fncEmailQuestion('"+thisData.empNo+"');\" class=\"btn action small\"><span>이메일 문의하기</span></a></div></td>"
            	+ "</tr>";
...
var fncSatiResultView = function(){
