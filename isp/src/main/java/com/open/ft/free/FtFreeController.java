...
public class FtFreeController {
    ...
    /** folderPath **/
    private final static String folderPath = "/ft/guest/free/";
	...
	public String form(@ModelAttribute("searchVO") FreeVO searchVO, Model model,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		}else if(procType.equals("reInsert")){ // 답글등록 
			searchVO.setFeAtchFileId(""); // 게시글의 첨부파일아이디는 비워야 함.  
			model.addAttribute("freeVO", searchVO); // view 에서 searchVO 로 지정된 값 넘김 
		
		}else if(procType.equals("reUpdate")){ // 답글수정 
			searchVO.setFeSeq(searchVO.getFeGroupSeq()); // 쿼리조건 groupSeq = seq 
			freeVO = (FreeVO) cmmnService.selectContents(searchVO, PROGRAM_ID+".replySelectContents"); // 조회조건 답글 
			model.addAttribute("freeVO", freeVO); 
		}
	...
	public String proc(@ModelAttribute("searchVO") FreeVO searchVO, Model model, SessionStatus status,@PathVariable String procType, HttpServletRequest request) throws Exception {
		...
		if(procType != null){
			...
			} else if (procType.equals("reInsert")) {
				cmmnService.insertContents(searchVO, PROGRAM_ID+".replyInsertContents");
				model.addAttribute("message", "답글이 등록되었습니다.");
				model.addAttribute("cmmnScript", "list.do");
				return "cmmn/execute";
				
			} else if (procType.equals("reUpdate") ) {			
				cmmnService.updateContents(searchVO, PROGRAM_ID); // 쿼리동일 		
			...
			} else if (procType.equals("reDelete")) {				
				cmmnService.deleteContents(searchVO, PROGRAM_ID);
