...
/**
	...
	public static Map<String, String> getMenuCd(final String url) throws Exception {
		...
		String regExpPttrn1 = "/([a-z]{2})/([a-z]+)/([a-zA-Z0-9]+)/([a-zA-Z0-9\\_]+\\.?)";
		Pattern ptn = Pattern.compile(regExpPttrn1);
		Matcher matcher = ptn.matcher(url);
		
		String regExpPttrn2 = "/([a-z]+)/([a-z]+)/([a-zA-Z0-9\\_]+\\.?)";
		Pattern ptn2 = Pattern.compile(regExpPttrn2);
		Matcher matcher2 = ptn2.matcher(url);
		
		if(matcher.find()) {
			infoMap.put("depth0", matcher.group(1)); //1차
			infoMap.put("depth1", matcher.group(2)); //1차
			infoMap.put("depth2", matcher.group(3)); //2차
			if(!matcher.group(4).contains(".")){
				infoMap.put("depth3", matcher.group(4)); //3차
			}
	...
	public static Map<String, String> getMicroMenuCd(final String url) throws Exception {
		Map<String, String> infoMap = new HashMap<String, String>();
		String regExpPttrn1 = "/([a-z]+)/([a-z]+)/([a-zA-Z0-9]+)/([a-zA-Z0-9]+\\.?)";
		Pattern ptn = Pattern.compile(regExpPttrn1);
		Matcher matcher = ptn.matcher(url);
		
		String regExpPttrn2 = "/([a-z]+)/([a-z]+)/([a-zA-Z0-9]+\\.?)";
		Pattern ptn2 = Pattern.compile(regExpPttrn2);
		Matcher matcher2 = ptn2.matcher(url);
